...
describe('getDiffRowsRenderLineCount', () => {
  it('counts unified rows one line at a time', () => {
    const diff = [
    ...
    assert.equal(getDiffRowsRenderLineCount(rows), rows.length);
  });

  it('counts side-by-side modified rows as two visual lines', () => {
    const diff = [
      'diff --git a/example.ts b/example.ts',
      'index 1111111..2222222 100644',
      '--- a/example.ts',
    ...
    const file = parseUnifiedDiffDocument(diff).files[0]!;
    const rows = projectDiffRows(file, 'side-by-side');
